...
public class DBMapper extends Mapper<LongWritable, Text, Text, Text> {
	...
	protected void map(LongWritable key, Text value, Context context) {
		try 
		{
			...
			String[] product = new String[9]; // (수정) 상품 테이블의 컬럼 수에 맞게 수정
			...
			while(token.hasMoreTokens()) {
				...
				// Driver에 명시한  new String[] {"pcode", "name", "brand", "price", "img", "pressure", "heat_time", "water_volume", "spec"} //table columns 이 순서와 동일하게!
				if (str[0].equals("pcode")) {
					...
					product[4] = str[1];
				} else if (str[0].equals("펌프압력")) {
					product[5] = str[1];
				} else if (str[0].equals("예열시간")) {
					product[6] = str[1];
				} else if (str[0].startsWith("용량")) {
					product[7] = str[1];
				} else {
			...
			product[8] = makeSpecToJson(specList); // 전체 스펙
