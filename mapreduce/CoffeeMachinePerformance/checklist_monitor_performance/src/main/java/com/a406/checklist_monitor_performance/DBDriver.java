...
//		);
		...
		DBInputFormat.setInput(job, DBInputWritable.class,
				"select pcode, price, pressure, heat_time, water_volume, spec, DENSE_RANK() over (order by price desc) as ranking from coffeemachine_product",	// query
				"select count(pcode) from coffeemachine_product"	// null 주면 오류남
		);

		// DBOutputWritable 포맷에 맞게 string 배열 수정
		// db의 컬럼명과 일치해야 함
		DBOutputFormat.setOutput(job, "coffeemachine_performance", // output table name
				new String[] { "pcode", "price", "brewing", "water_volume", "convenience" } // table columns
		);
