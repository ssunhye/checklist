...
//		);
		...
		DBInputFormat.setInput(job, DBInputWritable.class,
				"select pcode, price, size, pixel, hz, spec, DENSE_RANK() over (order by price desc) as ranking from monitor_product",	// query
				"select count(pcode) from monitor_product"	// null 주면 오류남
		);

		// DBOutputWritable 포맷에 맞게 string 배열 수정
		// db의 컬럼명과 일치해야 함
		DBOutputFormat.setOutput(job, "monitor_performance", // output table name
				new String[] { "pcode", "price", "size", "pixel", "screen_change", "convenience" } // table columns
		);
